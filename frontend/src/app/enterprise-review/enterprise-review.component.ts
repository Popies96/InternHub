...
export class EnterpriseReviewComponent implements OnInit {
  ...
  async downloadRecommendationAsPDF(): Promise<void> {
    ...
    doc.text('InternHub Performance Evaluation', pageWidth / 2, yPos, { align: 'center' });
    ...
    try {
      ...
      if (canvas) {
        ...
        if (chartImage) {
          ...
          doc.setFontSize(10);
          doc.setFont('helvetica', 'normal');
          doc.text('The following chart illustrates the performance metrics over time:', margin, yPos);
          yPos += 12;
      ...
      doc.text('Performance chart could not be included in this report', margin, yPos);
      yPos += 10;
    }
    ...
    doc.text('© ' + today.getFullYear() + ' InternHub. All rights reserved.',
    ...
    // Save the PDF
    const fileName = `InternHub_Performance_Report_${today.getFullYear()}${(today.getMonth()+1).toString().padStart(2, '0')}${today.getDate().toString().padStart(2, '0')}.pdf`;
    doc.save(fileName);
  }
  ...
  submitReport() {
    ...
    this.reviewService.reportReview(this.reportReviewId, this.selectedReason).subscribe({
      next: (response) => {

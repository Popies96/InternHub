...
export class CertificateService {
  ...
  getAllCertificatesForAuthenticatedIssuer(): Observable<Certificate[]> {
    return this.http.get<Certificate[]>(API_URL, { headers: this.getHeaders() })
      .pipe(catchError(this.handleError));
  }

  getCertificateById(id: number): Observable<Certificate> {
    return this.http.get<Certificate>(`${API_URL}/${id}`, { headers: this.getHeaders() })
      .pipe(catchError(this.handleError));
  }

  createCertificate(certificateData: Certificate): Observable<Certificate> {
    return this.http.post<Certificate>(API_URL, certificateData, { headers: this.getHeaders() })
      .pipe(
  ...
  getMyCertificates(): Observable<Certificate[]> {
    return this.http.get<Certificate[]>(`${API_URL}/my-certificates`, { headers: this.getHeaders() })
      .pipe(catchError(this.handleError));
  }

  getCertificateDetails(id: number): Observable<Certificate> {
    return this.http.get<Certificate>(`${API_URL}/${id}/details`, { headers: this.getHeaders() })
      .pipe(catchError(this.handleError));
  }

  updateCertificate(id: number, certificate: Certificate): Observable<Certificate> {
    return this.http.put<Certificate>(`${API_URL}/${id}`, certificate, { headers: this.getHeaders() })
      .pipe(catchError(this.handleError));
  }
  ...
  getAllInternships(): Observable<any[]> {
    return this.http.get<any[]>(`${API_URL}/internships/completed`, { headers: this.getHeaders() })
      .pipe(catchError(this.handleError));
  }

  getStudentsByInternship(internshipId: number): Observable<any[]> {
    return this.http.get<any[]>(`${API_URL}/internships/${internshipId}/students`, { headers: this.getHeaders() })
      .pipe(catchError(this.handleError));
  }

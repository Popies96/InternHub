...
updateTopic(id: number, userId: number, topic: any, file?: File): Observable<any> {
  const formData = new FormData();
  formData.append('topic', new Blob([JSON.stringify(topic)], { type: 'application/json' }));
  ...
  return this.http.post<any>(`${this.baseUrl}/update/${id}/${userId}`, formData);  // Use POST instead of PUT
}
...
incrementViewCount(id: number): Observable<any> {
  return this.http.post<any>(`${this.baseUrl}/view/${id}`, null);
    ...
    private getImageUrl(imagePath: string | undefined): string | undefined {
      return imagePath ? `${this.imageBaseUrl}/topic/image/${imagePath}` : undefined;
    }
    ...
    uploadImage(formData: FormData): Observable<string> {
      return this.http.post('http://localhost:8088/internhub/image/upload', formData, { responseType: 'text' });
    }
  }
...
export class CertificatesComponent implements OnInit {
  ...
  loadCertificates(): void {
    this.certificateService
      .getAllCertificatesForAuthenticatedIssuer()
      .subscribe({
        next: (data) => {
    ...
    this.filteredCertificates = this.certificates.filter(
      (cert) =>
        ...
        (cert.studentFirstName &&
          cert.studentFirstName.toLowerCase().includes(term)) ||
        (cert.studentLastName &&
          cert.studentLastName.toLowerCase().includes(term)) ||
        (cert.internshipTitle &&
          cert.internshipTitle.toLowerCase().includes(term)) ||
        (cert.issuerCompanyName &&
          cert.issuerCompanyName.toLowerCase().includes(term))
    );
  }
    ...
    if (confirm('Are you sure you want to delete this certificate?')) {
      this.certificateService.deleteCertificate(id).subscribe({
        next: () => {
          ...
          this.filteredCertificates = this.filteredCertificates.filter(
            (c) => c.id !== id
          );
        },

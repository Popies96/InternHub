...
export class AddCertificatesComponent implements OnInit {
  ...
  updateFormContext(): void {
    const selectedInternship = this.internships.find(
      (i) => i.id === this.certificateForm.value.internshipId
    );
    const selectedStudent = this.students.find(
      (s) => s.id === this.certificateForm.value.studentId
    );
  ...
  loadInternships(): void {
    this.certificateService.getAllInternships().subscribe({
      next: (internships) => {
        this.internships = internships.filter(

...
export class ChatappComponent implements OnInit {
  ...
  async startRecording() {
    ...
    try {
        ...
        const preferredMic = devices.find(device => device.kind === 'audioinput' && device.label.includes('your microphone name or a unique part of its ID'));
        ...
        this.snackBar.open('Microphone access denied or issue with device', 'Dismiss', { duration: 3000 });
    }
}
  ...
  private setupMediaRecorder(stream: MediaStream) {
    ...
    this.mediaRecorder.onstop = () => {
        ...
        console.log('Recording stopped, total chunks:', this.audioChunks.length, 'Blob size:', audioBlob.size);
        ...
        if (this.selectedUser && audioBlob.size > 0) { // Only upload if there's data
            this.uploadAudio(audioBlob);
        } else if (this.selectedUser && audioBlob.size === 0) {
            this.snackBar.open('No audio data recorded', 'Dismiss', { duration: 3000 });
        }
    };
...
private mergeMessages(existing: ChatMessage[], newMessages: ChatMessage[]): ChatMessage[] {
...
filefetchAudioFile(audioUrl: string): Observable<Blob> {
  ...
  return this.http.get(fullUrl, { 
    ...
    catchError(err => {
      ...
      if (this.snackBar) {
        this.snackBar.open('Failed to load audio', 'Dismiss', { duration: 3000 });
      }
...
private getFullAudioUrl(audioUrl: string): string {
  ...
  // Construct full URL
  return `http://localhost:8088/internhub/audio/${encodeURIComponent(audioUrl)}`;
}
// Example usage:
playAudioMessage(message: ChatMessage): void {
  ...
  this.filefetchAudioFile(message.audioUrl).subscribe({
    ...
    error: (err) => {
      console.error('Error fetching audio file:', err);
      this.snackBar.open('Failed to fetch audio', 'Dismiss', { duration: 3000 });
  ...
  uploadAudio(audioBlob: Blob) {
    ...
    this.http.post('http://localhost:8088/internhub/upload-audio', formData).subscribe({
      next: (response: any) => {

...
export class AiCertificateAssistantComponent implements OnInit {
  ...
  ngOnInit() {
    this.addBotMessage("Bonjour ! Je suis ici pour vous aider à rédiger votre certificat. Posez-moi vos questions.");
  }
  ...
  sendMessage() {
    ...
    this.aiService.getCertificateSuggestion(currentInput, this.formContext).subscribe({
      next: (response) => {
      ...
      error: (err) => {
        this.addBotMessage("Désolé, je n'ai pas pu obtenir de réponse. Veuillez réessayer.");
      },
      complete: () => {
  ...
  private autoFillForm(aiResponse: string) {
    if (aiResponse.includes('Contenu suggéré:')) {
      ...
      this.certificateForm!.get('certificateContent')!.setValue(suggestedContent.trim());
    }
  }
